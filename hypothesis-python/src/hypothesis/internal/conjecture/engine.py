    ...
    def test_function(self, data):
        ...
        if data.status >= Status.VALID:
            for k, v in data.target_observations.items():
                ...
                if (
                    k not in self.best_examples_of_observed_targets
                    or v
                    > self.best_examples_of_observed_targets[k].target_observations[k]
                    or (
                        v
                        == self.best_examples_of_observed_targets[
                            k
                        ].target_observations[k]
                        and sort_key(data.buffer)
                        < sort_key(self.best_examples_of_observed_targets[k].buffer)
                    )
                ):
