...
def test_advanced_integer_index_is_valid(shape, min_dims, min_side, dtype, data):
...
def test_advanced_integer_index_can_generate_any_pattern(shape, data):
    ...
    target = data.draw(
        nps.arrays(
            ...
            elements=st.integers(x.min(), x.max()),
            dtype=x.dtype,
        ),
        label="target",
    )
    find_any(
        nps.integer_array_indices(
            shape,
            min_dims=target.ndim,
            max_dims=target.ndim,
            min_side=min(target.shape),
            max_side=max(target.shape),
            dtype=np.dtype("int8"),
        ),
